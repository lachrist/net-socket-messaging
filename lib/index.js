...
const send = function (message) {
  if (message.length > MAX) {
    log("message too large %i", message.length);
    this.emit("error", new global_Error(`Cannot send message because its utf8 byte length is greater than ${MAX}, got: ${message.length}`));
    return null;
  }
